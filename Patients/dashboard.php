...
// Robust matching for patient's registered Barangay
$cleanPatientBrgy = trim((string) preg_replace('/\b(?:Brgy\.?|Barangay)\b/i', '', $patientBarangay));
$cleanPatientBrgy = trim($cleanPatientBrgy, ", .-#\t\n\r\0\x0B");
$cleanPatientSlug = strtolower(str_replace([' ', '-', '_', '.'], '', $cleanPatientBrgy));

foreach ($stations as $stItem) {
    if ((string) ($stItem['slug'] ?? '') === 'city-health') {
        continue;
    }
    $stBrgy = trim((string) preg_replace('/\b(?:Brgy\.?|Barangay)\b/i', '', (string) ($stItem['barangay'] ?? '')));
    $stSlug = strtolower(str_replace([' ', '-', '_', '.'], '', (string) ($stItem['slug'] ?? '')));
    if (
        ($cleanPatientBrgy !== '' && strcasecmp($stBrgy, $cleanPatientBrgy) === 0)
        || ($cleanPatientSlug !== '' && strcasecmp($stSlug, $cleanPatientSlug) === 0)
        || (strlen($cleanPatientBrgy) >= 3 && stripos($stBrgy, $cleanPatientBrgy) !== false)
        || (strlen($cleanPatientBrgy) >= 3 && stripos($cleanPatientBrgy, $stBrgy) !== false)
    ) {
        $selectedStation = $stItem;
        $stationSlug = (string) $stItem['slug'];
        $patientBarangay = (string) $stItem['barangay'];
        break;
    }
}

if ($selectedStation === null && !empty($stations)) {
    // Default to first public station
    foreach ($stations as $stItem) {
        if ((string) ($stItem['slug'] ?? '') !== 'city-health') {
            $selectedStation = $stItem;
            $stationSlug = (string) $stItem['slug'];
...
if ($bookedRef !== '') {
    ...
    if ($justBookedAppt !== null) {
        $alreadyInList = false;
        foreach ($patientAppointments as $pa) {
            if (($pa['appointment_code'] ?? $pa['reference_code'] ?? '') === ($justBookedAppt['appointment_code'] ?? $justBookedAppt['reference_code'] ?? '')) {
                $alreadyInList = true;
                break;
            }
        }
        if (!$alreadyInList) {
            array_unshift($patientAppointments, $justBookedAppt);
        }
    }
}
...
if (!function_exists('iconSvg')) {
    ...
    {
        $icons = [
            'bell' => '<svg width="16" height="16" viewBox="0 0 24 24" style="width:16px;height:16px;max-width:16px;max-height:16px;display:inline-block;vertical-align:middle;" aria-hidden="true"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
            ...
            'syringe' => '<svg width="15" height="15" viewBox="0 0 24 24" style="width:15px;height:15px;max-width:15px;max-height:15px;display:inline-block;vertical-align:middle;" aria-hidden="true"><path d="m14 4 6 6M5 19l7.5-7.5m-3-3L17 16m-9 5-3 0 0-3 9-9 3 3-9 9Z" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
            ...
            'baby' => '<svg width="14" height="14" viewBox="0 0 24 24" style="width:14px;height:14px;max-width:14px;max-height:14px;display:inline-block;vertical-align:middle;" aria-hidden="true"><path d="M9.5 7.5c0-1.66 1.34-3 3-3 1.1 0 2.07.6 2.59 1.49M8.5 15a4.5 4.5 0 1 0 8.99 0A4.5 4.5 0 0 0 8.5 15Zm2-1h.01m4.98 0h.01M11 17c.35.48.91.8 1.5.8s1.15-.32 1.5-.8" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
            ...
            'calendar' => '<svg width="16" height="16" viewBox="0 0 24 24" style="width:16px;height:16px;max-width:16px;max-height:16px;display:inline-block;vertical-align:middle;" aria-hidden="true"><rect width="18" height="18" x="3" y="4" rx="2" ry="2" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><line x1="16" x2="16" y1="2" y2="6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><line x1="8" x2="8" y1="2" y2="6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><line x1="3" x2="21" y1="10" y2="10" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
            'sparkle' => '<svg width="16" height="16" viewBox="0 0 24 24" style="width:16px;height:16px;max-width:16px;max-height:16px;display:inline-block;vertical-align:middle;" aria-hidden="true"><path d="m12 3 1.9 5.3a2 2 0 0 0 1.2 1.2l5.3 1.9-5.3 1.9a2 2 0 0 0-1.2 1.2l-1.9 5.3-1.9-5.3a2 2 0 0 0-1.2-1.2l-5.3-1.9 5.3-1.9a2 2 0 0 0 1.2-1.2z" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
            ...
            'x' => '<svg width="16" height="16" viewBox="0 0 24 24" style="width:16px;height:16px;max-width:16px;max-height:16px;display:inline-block;vertical-align:middle;" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
            'check' => '<svg width="16" height="16" viewBox="0 0 24 24" style="width:16px;height:16px;max-width:16px;max-height:16px;display:inline-block;vertical-align:middle;" aria-hidden="true"><polyline points="20 6 9 17 4 12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
            'mail' => '<svg width="16" height="16" viewBox="0 0 24 24" style="width:16px;height:16px;max-width:16px;max-height:16px;display:inline-block;vertical-align:middle;" aria-hidden="true"><rect width="20" height="16" x="2" y="4" rx="2" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
            'lock' => '<svg width="16" height="16" viewBox="0 0 24 24" style="width:16px;height:16px;max-width:16px;max-height:16px;display:inline-block;vertical-align:middle;" aria-hidden="true"><rect width="18" height="11" x="3" y="11" rx="2" ry="2" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M7 11V7a5 5 0 0 1 10 0v4" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
            'eye' => '<svg width="16" height="16" viewBox="0 0 24 24" style="width:16px;height:16px;max-width:16px;max-height:16px;display:inline-block;vertical-align:middle;" aria-hidden="true"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><circle cx="12" cy="12" r="3" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
            'history' => '<svg width="16" height="16" viewBox="0 0 24 24" style="width:16px;height:16px;max-width:16px;max-height:16px;display:inline-block;vertical-align:middle;" aria-hidden="true"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M3 3v5h5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M12 7v5l4 2" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
            'filter' => '<svg width="16" height="16" viewBox="0 0 24 24" style="width:16px;height:16px;max-width:16px;max-height:16px;display:inline-block;vertical-align:middle;" aria-hidden="true"><polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
            'menu' => '<svg width="16" height="16" viewBox="0 0 24 24" style="width:16px;height:16px;max-width:16px;max-height:16px;display:inline-block;vertical-align:middle;" aria-hidden="true"><line x1="3" y1="12" x2="21" y2="12" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"/><line x1="3" y1="6" x2="21" y2="6" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"/><line x1="3" y1="18" x2="21" y2="18" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"/></svg>',
            'logout' => '<svg width="16" height="16" viewBox="0 0 24 24" style="width:16px;height:16px;max-width:16px;max-height:16px;display:inline-block;vertical-align:middle;" aria-hidden="true"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><polyline points="16 17 21 12 16 7" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><line x1="21" y1="12" x2="9" y2="12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>',
        ];
